...
def hhd_init(host="127.0.0.1", port=1883):
    try:
        ...
        if HHhdlist.device_mqtt_status:
            ...
            if heart_interval is None or heart_interval == 0:
                HStategrid.Heartbeat_class = HStategrid.Heartbeat_info()
            else:
                HStategrid.Heartbeat_class = HStategrid.Heartbeat_info(heart_interval, HStategrid.get_DeviceInfo("heart_timeout_num"))
            HStategrid.Device_class = HStategrid.Device_info(HStategrid.Device_ID)
        ...
        if HHhdlist.device_mqtt_status:
            ...
            if not HHhdlist.platform_device_data:
                ...
                if HHhdlist.platform_device_data.empty():
                    ...
                    msg = HHhdlist.platform_device_data.get()
                    HSyslog.log_info(f"__platform_device_data Faild: {msg}")
        ...
        if warnSum < 0 or faultSum < 0:
            ...
            if faultSum > 0:
                for info in faultVal:
                    ...
                    stop_fault_code = HHhdlist.get_fault_reason(fault_id)
                    HStategrid.Device_class.set_system_fault_code(stop_fault_code)
                HStategrid.Device_class.set_work_status(HStategrid.Device_Work_Status.Fault.value)
            else:
                HStategrid.Device_class.set_system_fault_code(0x00000000)
                HStategrid.Device_class.set_work_status(HStategrid.Device_Work_Status.Service.value)

            # if warnSum > 0:
            #     HStategrid.Device_class.set_system_fault_code(0x20000000)
            # else:
            #     HStategrid.Device_class.set_system_fault_code(0x00000000)
        else:
            HStategrid.Device_class.set_system_fault_code(0x00000000)
        ...
        if HStategrid.connect_status:
            for gun_id in HHhdlist.gun.keys():
                if HHhdlist.gun.get(gun_id).get(6) == 1 and HStategrid.Gun_list[gun_id].get_gun_connect_status() == HStategrid.Gun_Connect_Status.Not_Connect.value:
                    HStategrid.Gun_list[gun_id].set_gun_status(HStategrid.Gun_Status.Charge_preparation.value)
                    HStategrid.Gun_list[gun_id].set_gun_connect_status(HStategrid.Gun_Connect_Status.Connect.value)
                    HStategrid.Gun_list[gun_id].set_gun_work_status(HStategrid.Gun_Work_Status.Default.value)
                    ...
                    HStategrid.Gun_list[gun_id].set_gun_status(HStategrid.Gun_Status.Server_end.value)
...
def _hqc_main_event_notify_request_charge(msg_body_dict: dict):  # 充电请求消息
    ...
    try:
        ...
        for gun_info in HStategrid.Gun_list:
            HSyslog.log_info(f"{gun_info.gun_id + 1}: {gun_info.get_gun_charge()}")
            HSyslog.log_info(f"{gun_info.gun_id + 1}: {gun_info.get_gun_charge_order()}")
            HSyslog.log_info(f"{gun_info.gun_id + 1}: {gun_info.get_gun_charge_reserve()}")
    except Exception as e:
    ...
    try:
        if info == HHhdlist.control_charge.start_charge.value:
            ...
            msg = {
                ...
                'aux_power_type': 0xFF,
                'multi_mode': 0xFF,
                'user_id': HStategrid.Gun_list[gun_id].get_gun_charge("user_card_id"),
                'appointment_time': 0,
                'cloud_session_id': HStategrid.Gun_list[gun_id].get_gun_charge("charge_id"),
                'account_info': {
                    'stop_code': "000000",
                    'balance': HHhdlist.get_balance(HStategrid.Gun_list[gun_id].get_gun_charge("electricity_available_balance")) + HHhdlist.get_balance(HStategrid.Gun_list[gun_id].get_gun_charge("server_available_balance")),
                    'billing': 0,
                    'overdraft_limit': 0,
                    'electric_discount': 0,
                    'service_discount': 0,
...
def _hqc_main_event_notify_check_vin(msg_body_dict: dict):  # 车辆VIN鉴权消息
    ...
    try:
        ...
        HStategrid.Gun_list[gun_id].set_gun_charge({"gun_charge_mode": gun_charge_mode})
        ...
        if start_source == 4 and start_source == HStategrid.Gun_list[gun_id].get_gun_charge("start_source"):
            ...
            HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_70.value, auth_data])
        elif start_source == 7 and start_source == HStategrid.Gun_list[gun_id].get_gun_charge("start_source"):

            auth_data = {
                "gun_id": gun_id,
            }
            HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_7D.value, auth_data])
        else:
            pass
...
def _hqc_main_event_reply_check_vin(msg_body_dict: dict):  # 车辆VIN鉴权应答消息
    ...
    try:
        ...
        if gun_charge_mode == 0:
            ...
            HStategrid.Gun_list[gun_id].set_gun_charge_mode(HStategrid.Gun_Charge_Mode.Two_Gun.value)
            HStategrid.Gun_list[gun_id + 1].set_gun_charge_mode(HStategrid.Gun_Charge_Mode.Two_Gun.value)
            ...
            HStategrid.Gun_list[gun_id].set_gun_charge_mode(HStategrid.Gun_Charge_Mode.Two_Gun.value)
            HStategrid.Gun_list[gun_id + 1].set_gun_charge_mode(HStategrid.Gun_Charge_Mode.Two_Gun.value)
            HStategrid.Gun_list[gun_id + 2].set_gun_charge_mode(HStategrid.Gun_Charge_Mode.Two_Gun.value)
            HStategrid.Gun_list[gun_id + 1].copy_gun_charge(gun_id)
...
def _hqc_main_event_notify_charge_record(msg_body_dict: dict):  # 充电记录消息
    ...
    try:
        if cloud_session_id == HStategrid.Gun_list[gun_id].get_gun_charge("cloud_session_id") and HStategrid.Gun_list[gun_id].get_gun_charge("cloud_session_id") is not None:
            HStategrid.Gun_list[gun_id].set_gun_status(HStategrid.Gun_Status.Stop_charge.value)

            if HStategrid.Gun_list[gun_id].get_gun_charge("cmd_id_stop") is not None:
                info = {
                    "cmd_id": HStategrid.Gun_list[gun_id].get_gun_charge("cmd_id_stop"),
                    "cmd_nums": HStategrid.Gun_list[gun_id].get_gun_charge("cmd_nums_stop"),
                    "param_type": 2,
                    "param_num": 1,
                    "param_list": [{
                        "param_no": gun_id + 1,
                ...
                HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_15.value, info])
            info = {
                "gun_id": gun_id,
            }
            HStategrid.Gun_list[gun_id].set_gun_charge_order(msg_body_dict)
            HStategrid.Gun_list[gun_id].set_gun_charge_reserve()
            HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_34.value, info])
            HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_35.value, info])
            HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_36.value, info])
            HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_3A.value, info])
            HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_3F.value, info])
            HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_7F.value, info])
        else:
            info = {
                "gun_id": gun_id,
...
def _hqc_main_event_notify_charge_account(msg_body_dict: dict):  # 充电结算消息
    ...
    try:
        info = {
            "gun_id": gun_id,
            "cloud_session_id": cloud_session_id,
            "user_id": HStategrid.Gun_list[gun_id].get_gun_charge_reserve("user_id"),
            "balance": 0,
            "energy": HStategrid.Gun_list[gun_id].get_gun_charge_reserve("total_energy"),
            "electric_cost": HStategrid.Gun_list[gun_id].get_gun_charge_reserve("charge_electricity_cost"),
            "service_cost": HStategrid.Gun_list[gun_id].get_gun_charge_reserve("charge_server_cost"),
            "total_cost": HStategrid.Gun_list[gun_id].get_gun_charge_reserve("charge_cost"),
        }
...
def _hqc_main_event_notify_charge_session(msg_body_dict: dict):  # 充电会话消息
    ...
    try:
        if HStategrid.Gun_list[gun_id].get_gun_charge("charge_id") == cloud_session_id:
            ...
            HStategrid.Gun_list[gun_id].set_gun_work_status(HStategrid.Gun_Work_Status.Charging.value)
            ...
            if HStategrid.Gun_list[gun_id].get_gun_charge("start_source") == start_source:
                ...
                #     "result": 1,
                # }
                # HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_7A.value, info])
                info = {
                    "gun_id": gun_id,
                }
                HHhdlist.device_platform_data.put([HStategrid.tpp_mqtt_cmd_enum.tpp_cmd_type_35.value, info])
...
def _hqc_sys_upgrade_notify_process_version(msg_body_dict: dict):  # 读取进程版本号消息
    process_type_list = msg_body_dict.get("process_type", [])
    try:
        for process_type in process_type_list:
            info = {
                "type": process_type,
            }
            topic = HHhdlist.topic_hqc_sys_upgrade_notify_process_version
            app_publish(topic, info)
    except Exception as e:
        HSyslog.log_error(f"_hqc_sys_upgrade_notify_process_version error: {e}")


def _hqc_sys_upgrade_reply_process_versio(msg_body_dict: dict):  # 读取进程版本号应答消息
    type = msg_body_dict.get('type', -1)  # int
    soft_version = msg_body_dict.get('soft_version', -1)  # int
    try:
        hard_version = HStategrid.get_VerInfoEvt(HHhdlist.device_ctrl_type.DTU.value)[0]
        if hard_version is None or hard_version == "":
            hard_version = ""
        if type == 0:
            pass
        if type == 1:
            HStategrid.save_VerInfoEvt(0, HHhdlist.device_ctrl_type.UI.value, hard_version, soft_version)
        if type == 2:
            HStategrid.save_VerInfoEvt(0, HHhdlist.device_ctrl_type.Main.value, hard_version, soft_version)
        if type == 3:
            HStategrid.save_VerInfoEvt(0, HHhdlist.device_ctrl_type.Advertisement.value, hard_version, soft_version)
    except Exception as e:
        HSyslog.log_error(f"_hqc_sys_upgrade_reply_process_versio error: {e}")
...
app_func_dict = {
    ...
    '/hqc/main/event-notify/charge-elec': {'name': '/hqc/main/event-notify/charge-elec', 'isSubscribe': False, 'qos': 2, 'func': _hqc_main_event_notify_charge_elec},  # 充电电量冻结消息
    ...
    '/hqc/cloud/event-reply/recharge': {'name': '/hqc/cloud/event-reply/recharge', 'isSubscribe': False, 'qos': 2, 'func': _hqc_cloud_event_reply_recharge},  # 账户充值应答消息
    '/hqc/cloud/event-notify/balance-query': {'name': '/hqc/cloud/event-notify/balance-query', 'isSubscribe': False, 'qos': 2, 'func': _hqc_cloud_event_notify_balance_query},  # 账户余额查询消息
    ...
    '/hqc/cloud/event-notify/request-rate': {'name': '/hqc/cloud/event-notify/request-rate', 'isSubscribe': False, 'qos': 2, 'func': _hqc_cloud_event_notify_request_rate},  # 充电系统费率请求消息
    ...
    '/hqc/cloud/event-reply/query-rate': {'name': '/hqc/cloud/event-reply/query-rate', 'isSubscribe': False, 'qos': 2, 'func': _hqc_cloud_event_reply_query_rate},  # 充电系统费率查询结果消息
    '/hqc/cloud/event-notify/request-gunrate': {'name': '/hqc/cloud/event-notify/request-gunrate', 'isSubscribe': False, 'qos': 2, 'func': _hqc_cloud_event_notify_request_gunrate},  # 充电枪费率请求消息
    ...
    '/hqc/cloud/event-reply/m1-secret': {'name': '/hqc/cloud/event-reply/m1-secret', 'isSubscribe': False, 'qos': 2, 'func': _hqc_cloud_event_reply_m1_secret},  # M1卡密钥更新结果消息
    ...
    '/hqc/main/event-reply/update-rate': {'name': '/hqc/main/event-reply/update-rate', 'isSubscribe': False, 'qos': 2, 'func': _hqc_main_event_reply_update_rate},  # 充电系统费率同步应答消息
